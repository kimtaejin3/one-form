...
| **입사지원서 빌더**   | 프로필에서 이력서·경력기술서·자기소개서를 만들고, 낼 문서만 골라 한 개의 PDF로 병합 |
| **채팅 편집**         | 내용도 스타일(템플릿·폰트·강조색·여백)도 채팅으로 바꾼다 — PDF는 매번 다시 렌더 |
| **채용공고 추천**     | 프로필↔공고를 임베딩·LLM으로 대조해 매칭률·근거 표시 + 상세페이지   |
| **양식 변환**         | 자사 지원 양식 업로드 → 프로필 필드로 자동 매핑                     |

> 크롬 익스텐션(외부 채용 사이트 폼 자동 입력)은 계획 단계다 — [docs/TODO.md](docs/TODO.md) 참고.

## 화면

**입사지원서 빌더 — 템플릿 선택 (미리보기는 샘플 데이터)**
![템플릿 선택](docs/screenshots/04-resume-templates.png)
...
<td width="50%"><b>양식 변환</b><br><img src="docs/screenshots/06-forms.png" width="100%" alt="양식 변환"></td>
</tr>
</table>
...
낸다. 나머지 AI 기능(입사지원서 채팅 편집·자기소개서 초안 등)은 아직 목(mock)이다.
